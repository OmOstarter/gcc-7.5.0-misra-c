...
#define SYNTAX_WARNING_AT(loc, msgid) \
  ...
  while(0)
unsigned int hash_value_compute(char *key)						//5.4
{
        unsigned int value=0;
        while(*key!='\0')
        {
                value=value+(value << 5)+((*key)-'\0');
                key++;
        }
...
int hash_search_macro(char *key,int value)
{
	while(HASH_TABLE[value].used_flag!=0)
	{
		if(strcmp(HASH_TABLE[value].key,key)==0)
		{
			if(strcmp(key,"__GCC_HAVE_SYNC_COMPARE_AND_SWA")!=0)
				return 1;
		}
		value=value+1;
		if(value==HASH_SIZE)
	      ...
	      _cpp_buff **pragma_buff, unsigned *num_args)
{
  ...
  unsigned char *misra_macro_temp = NULL;
  int misra_preprocesser = 0;
...
#define ARG_TOKENS_EXTENT 1000
  ...
  arg = args, argc = 0;
  if (node->value.macro->fun_like && CPP_OPTION(pfile, Wmisra_cpp_trigger)) { // MISRA-C 20.6
	misra_macro_temp = (unsigned char *)pfile->buffer->cur;
	while (*misra_macro_temp) {
		if (*misra_macro_temp == '\n' || *misra_macro_temp == '\t' || *misra_macro_temp == ' ') {
			misra_macro_temp++;
		} else if (*misra_macro_temp == '#') {
			misra_macro_temp++;
			misra_preprocesser = 1;
			break;
		} else {
			break;
		}
	...
	if (misra_preprocesser && *misra_macro_temp) {
...
/* 判斷這個宏是不是 INTn_C / UINTn_C / INTMAX_C / UINTMAX_C 類型。 */
static bool
misra_get_intconst_macro_info (cpp_reader *pfile,
                               ...
                               unsigned *bits)
{
  ...
  if (strcmp (name, "INTMAX_C") == 0)
    {
      *is_unsigned = false;
      *bits = CPP_OPTION (pfile, precision);
      return true;
    }

  if (strcmp (name, "UINTMAX_C") == 0)
    {
      *is_unsigned = true;
      *bits = CPP_OPTION (pfile, precision);
      return true;
...
/* === MISRA C:2025 Rule 7.6: 取得 target int 的位元寬度 ======================= */
static unsigned
misra_get_int_type_width (cpp_reader *pfile)
{
  return CPP_OPTION (pfile, int_precision);
}
/* === end MISRA C:2025 Rule 7.6 helper ================================= */


static cpp_num
misra_make_max_unsigned_value (unsigned int value_bits)
{
  const unsigned int part_bits = sizeof (cpp_num_part) * CHAR_BIT;
  cpp_num result;
  ...
  if (value_bits >= part_bits * 2)
    {
      result.high = ~(cpp_num_part) 0;
      result.low = ~(cpp_num_part) 0;
    }
  else if (value_bits > part_bits)
    {
      result.high = ((cpp_num_part) 1 << (value_bits - part_bits)) - 1;
      result.low = ~(cpp_num_part) 0;
    }
  else if (value_bits == part_bits)
    result.low = ~(cpp_num_part) 0;
  else if (value_bits > 0)
    result.low = ((cpp_num_part) 1 << value_bits) - 1;

  return result;
}

static bool
misra_cpp_num_gt (cpp_num lhs, cpp_num rhs)
{
                                        ...
                                        cpp_num *value)
{
  ...
  flags = cpp_classify_number (pfile, tok, NULL, tok->src_loc);

  if ((flags & CPP_N_CATEGORY) != CPP_N_INTEGER)
    return false;

  /* Rule 7.5 要求 unsuffixed integer literal。libcpp 會把 U/L/LL/I/J
     這些 suffix 反映在 flags 裡，因此只要有 suffix 就拒絕。 */
  if ((flags & CPP_N_UNSIGNED) != 0
      || (flags & CPP_N_IMAGINARY) != 0
      || (flags & CPP_N_WIDTH) != CPP_N_SMALL)
    return false;

  *value = cpp_interpret_integer (pfile, tok, flags);
                                        ...
                                        source_location invoc_loc)
{
  ...
  if (!misra_get_intconst_macro_info (pfile, uname, &is_unsigned, &bits))
  ...
  maxv = misra_make_max_unsigned_value (is_unsigned ? bits : bits - 1);

  if (misra_cpp_num_gt (value, maxv))
    {
      source_location loc = tok->src_loc;
      cpp_warning_with_line (pfile, CPP_W_NONE, loc, 0,
                             "MISRA C:2025 Rule 7.5");
...
create_iso_definition (cpp_reader *pfile, cpp_macro *macro)
{
  ...
  unsigned char *misra_macro_20_11;
  int cpp_hash = 0;
...
_cpp_create_definition (cpp_reader *pfile, cpp_hashnode *node)
{
  ...
  if(node && macro->syshdr==0 && CPP_OPTION(pfile,Wmisra_cpp_trigger))
  {
    ...
    // if(cpp_sys_macro_p(pfile))
		...
		//if(MAIN_FILE_P (linemap_check_ordinary
			    //(linemap_lookup (pfile->line_table,
					    // macro->line))) && node->type == NT_MACRO && !(node->flags & NODE_BUILTIN))
						//if(NODE_NAME(node)[0])
				//cpp_warning_with_line (pfile, CPP_DL_NOTE, node->value.macro->line, 0,
			 //"string: %s\n",NODE_NAME(node));
			//fprintf(stderr,"%s\n",NODE_NAME(node));
  if (CPP_OPTION (pfile, traditional))
				{
  ...
  if (!ok)
    ...
    return ok;
  //if( ! macro->syshdr && ! cpp_in_system_header (pfile))
	//	fprintf(stderr,"%s\n",NODE_NAME(node));
  if (node->type == NT_MACRO)
    { 
    //fprintf(stderr,"%s\n",NODE_NAME(node));

      if (CPP_OPTION (pfile, warn_unused_macros) || CPP_OPTION(pfile, Wmisra_cpp_trigger))
  ...
  {
	  // cpp_warning_with_line (pfile, CPP_DL_NOTE, node->value.macro->line, 0,"string: %s\n",NODE_NAME(node));
