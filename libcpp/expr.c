...
#define SYNTAX_WARNING_AT(loc, msgid) \
  ...
  while(0)
#define SYNTAX_WARNING_AT2(loc, msgid) \
  do { cpp_error_with_line (pfile, CPP_DL_NOTE, (loc), 0, msgid); goto syntax_ok; } \
  while(0)
/* Subroutine of cpp_classify_number.  S points to a float suffix of
   length LEN, possibly zero.  Returns 0 for an invalid suffix, or a
   flag vector (of CPP_N_* bits) describing the suffix.  */

int lowercase_l_suffix = 0;
...
int misra_check_rule_7_2;
int misra_rule_7_2_location;
unsigned int
		     ...
		     const char **ud_suffix, source_location virtual_location)
{
  ...
  bool seen_digit;
  bool seen_digit_sep;
  bool misra_flags_check_Decimal_point = false;                      // check misra-c rule
  if (ud_suffix)
    *ud_suffix = NULL;
  ...
  for (;;)
	...
	{
	  misra_flags_check_Decimal_point = true;
