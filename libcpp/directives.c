...
lex_macro_node (cpp_reader *pfile, bool is_def_or_undef)
{
  ...
  source_location src_loc1;                                         // misra-c 21.01
  if (token->type == CPP_NAME)
	...
	src_loc1 = pfile->directive_line;
      else
	src_loc1 = pfile->line_table->highest_line;
    }
  /* We don't want to refer to a token before the beginning of the
    ...
    {
      src_loc1 = pfile->cur_token[-1].src_loc;
    }
	...
	cpp_error (pfile, CPP_DL_ERROR,
	  ...
	  else if(is_def_or_undef && node == pfile->spec_nodes.n__)
	cpp_warning (pfile,CPP_W_NONE,"\"_\" cannot be used in macro");
...
int misra_compare_string(char *str1, char *str2)
{
	while (*str1 && *str2) {
		if (*str1 != *str2) {
			return 1;
...
do_define (cpp_reader *pfile)
{
  ...
  char *misra_tmp_macro_name = (char *)node->ident.str;
  int c_keyword_size = 0;
  int c_keyword_number = 0;
  int misra_c_define = 0;   // misra-c 20.4
  unsigned char *misra_macro_20_10;
  //fprintf(stderr, (char *)node->ident.str);
  const char *misra_tmp_c_keyword[] = {
  	"static", "unsigned", "long", "const", "extern", "register", 
	"typedef", "short", "inline", "volatile", "signed", "auto", "restrict",
	"noreturn", "atomic", "int", "char", "float", "double", "void",
	"enum", "struct", "union", "if", "else", "while", "do", "for",
	"switch", "case", "default", "break", "continue", "return", "goto",
	"sizeof", "define", "bool"
  };
  c_keyword_size = sizeof(misra_tmp_c_keyword);
  c_keyword_number = c_keyword_size / sizeof(char *);
...
int compare_misra_header_string(char *str1, char *str2)
{
	while (*str1 && *str2) {
		if (*str1 != *str2) {
			return 1;
	       ...
	       const cpp_token ***buf, source_location *location)
{
  ...
  *location = header->src_loc;
  //misra_c_check_header_21_3 = misra_c_check_header_21_4 = misra_c_check_header_21_5 = misra_c_check_header_21_7_8 = (char *)header->val.str.text;
  ...
  misra_c_tmp_fname = fname;
  
  while (*misra_c_tmp_fname) {
  	if (*misra_c_tmp_fname == ',' || 
	    *misra_c_tmp_fname == '\\' || 
	    *misra_c_tmp_fname == '*' || 
	    *misra_c_tmp_fname == '/' || 
	    *misra_c_tmp_fname == '\'' || 
