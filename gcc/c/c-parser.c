...
extern bool trigger_18_7;

extern long long int misra_enum_array_08_12[256];
extern int misra_enum_index;
int misra_14_4;    // MISRA-C 14-4
int misra_14_4_bool_type;
int misra_18_4;
int misra_15_4;
...
int misra_open_paren; // check misra 12.3
int misra_open_brace;
...
extern int misra_check_rule_7_2;
static void
  ...
  while (true)
    {
     ...
     implicit int suffices).  */
	  ...
	  misra_extern_8_11 = 0;
	  inform(misra_extern_8_11_location, "[MISRA C:2025 Rule 8.11] when an array with external linkage is declared, its size should be explicitly stated\n");
      ...
      || c_parser_next_token_is_keyword (parser, RID_IN))
    {
	  ...
	  if (Wmisra_c_trigger && d->typed.type->base.code == INTEGER_TYPE) {
		if (d->typed.type->base.u.bits.unsigned_flag && misra_check_rule_7_2) {
			inform(init_loc, "[MISRA C:2025 Rule 7.2] a suffix shall be applied to all integer constants not of type int\n");
          ...
          if(trigger_9_3 == 1 && init_loc && Wmisra_c_trigger){
            inform(init_loc,"[MISRA C:2025 Rule 9.3] arrays shall not be partially initialized\n");
            trigger_9_3 = 0;
            }
          if(trigger_9_5 == 1 && init_loc && Wmisra_c_trigger){
            inform(init_loc,"[MISRA C:2025 Rule 9.5] when designated initializers are used the array size shall be specified explicitly\n");
            trigger_9_5 = 0;
            }
          if(trigger_18_7 == 1 && init_loc && Wmisra_c_trigger){
            inform(init_loc,"[MISRA C:2025 Rule 18.7] flexible array members shall not be declared\n");
            trigger_18_7 = 0;
            }
          flag_sanitize = flag_sanitize_save;
          finish_init ();
            ...
            enum c_lookahead_kind la)
{
  ...
  while (c_parser_next_token_is (parser, CPP_NAME)
    ...
    inform(c_parser_peek_token (parser)->location,"[MISRA C:2025 Rule 19.2] the union keyword should not be used\n");
      if (!typespec_ok)
        goto out;
      attrs_ok = true;
      seen_type = true;
...
c_parser_enum_specifier (c_parser *parser)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
      ...
      while (true)
    {
      ...
      postfix_attrs = c_parser_attributes (parser);
      if (Wmisra_c_trigger)
      for (int i = 0, temp; i < misra_enum_index; i += 2) {
          if (!i) {
                temp = *misra_enum_array_08_12;
                continue;
          } else if (temp == *(misra_enum_array_08_12 + i)) {
                inform(misra_enum_array_08_12[i + 1], "[MISRA C:2025 Rule 8.12] implicitly-specified enumeration constants shall be unique\n");
		inform(misra_enum_array_08_12[i - 1], "[MISRA C:2025 Rule 8.12] implicitly-specified enumeration constants shall be unique (implicit location)\n");
          }
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE))
    {
	  ...
	  //inform(parser->tokens->location, "[MISRA C:2025 Rule 8.11] when an array with external linkage is declared, its size should be explicitly stated\n");
      }
...
{
    //HowManyTimes(c_parser_initializer);
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
...
static struct c_expr
c_parser_braced_init (c_parser *parser, tree type, bool nested_p,                   ///{} 9.3
              struct obstack *outer_obstack)
{
    //HowManyTimes(c_parser_braced_init);
  struct c_expr ret;
...
static void
c_parser_initelt (c_parser *parser, struct obstack * braced_init_obstack)                        //9.4
{
  /* Parse any designator or designator list.  A single array
     designator may have the subsequent "=" omitted in GNU C, but a
     longer list or a structure member designator may not.  */
    //HowManyTimes(c_parser_initelt);
  ...
  if (c_parser_next_token_is (parser, CPP_NAME)
    ...
    {
        //debug_tree(c_parser_expr_no_commas (parser, NULL).value);
      /* des_seen is 0 if there have been no designators, 1 if there
     has been a single array designator and 2 otherwise.  */                       //6
        //HowManyTimes(cpp_name_else);
      int des_seen = 0;
         ...
         || c_parser_next_token_is (parser, CPP_DOT))
    {
      ...
      if (c_parser_next_token_is (parser, CPP_DOT))
          ...
          else
        {
          ...
          c_parser_error (parser, "expected identifier");
          c_parser_skip_until_found (parser, CPP_COMMA, NULL);
        //inform(loc,"c_parser_initelt");
          process_init_element (input_location, init, false,
                    braced_init_obstack);
            //////////////////////////////////////////////////////////////////////
          //printf("process_init_element Line:4500\n");
            //////////////////////////////////////////////////////////////////////
          return;
        }
        }
      else                                                 //[...]
        {
          ...
          location_t array_index_loc = UNKNOWN_LOCATION;
            loc_9_4=array_index_loc;
            //inform(array_index_loc,"1");
          ...
          if (des_prev == 0 && c_dialect_objc ())
        {
          ...
          rec = first;
          while (c_parser_next_token_is (parser, CPP_COMMA))                                    //after initialization expect next declaration
            {
              struct c_expr next;
...
//fprintf(stderr,"2\n");
          ...
          array_index_loc = c_parser_peek_token (parser)->location;
            //loc_9_4=array_index_loc;
            //if(array_index_loc==UNKNOWN_LOCATION)
            //inform(array_index_loc,"array_index_loc");
          first = c_parser_expr_no_commas (parser, NULL).value;
        //debug_tree(first);
          mark_exp_read (first);
      ...
      if (c_parser_next_token_is (parser, CPP_EQ))
          ...
          else
        {
          ...
          process_init_element (input_location, init, false,
                    braced_init_obstack);
            //////////////////////////////////////////////////////////////////////
          //printf("process_init_element Line:4636\n");
            //////////////////////////////////////////////////////////////////////
          return;
        }
        }
...
//fprintf(stderr,"2\n");                                 //set_desinator
  ...
  process_init_element (loc, init, false, braced_init_obstack);
            ...
            //////////////////////////////////////////////////////////////////////
                        //call rule 9.3 9.4 9.5 check function//
            //////////////////////////////////////////////////////////////////////
}
...
//if(Wmisra_c_trigger)
  //inform(brace_loc,"c_parser_compound_statement");
...
c_parser_compound_statement_nostart (c_parser *parser)
{
  ...
  //printf("c_parser_compound_statement_nostart\n");
  if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
    {
      c_parser_consume_token (parser);
      return;
                 ...
                 vec<tree> *chain)
{
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    case RID_IF:
      if(Wcontrolflow)
      inform(loc, "c_parser_if_statement");
      c_parser_if_statement (parser, if_p, chain);
      break;
    case RID_SWITCH:
      if(Wcontrolflow)
      inform(loc, "c_parser_switch_statement");
      c_parser_switch_statement (parser, if_p);
      break;
    case RID_WHILE:
      if(Wcontrolflow)
      inform(loc, "c_parser_while_statement");
      c_parser_while_statement (parser, false, if_p);
      break;
    case RID_DO:
      if(Wcontrolflow)
      inform(loc, "c_parser_do_statement");
      c_parser_do_statement (parser, false);
      break;
    case RID_FOR:
      if(Wcontrolflow)
      inform(loc, "c_parser_for_statement");
    ...
    case RID_GOTO:
      ...
      if(Wmisra_c_trigger)
        {
            inform(loc,"[MISRA C:2025 Rule 15.1] the goto statement should not be used\n");
        }
    ...
    return_is_used = return_is_used + 1;
      ...
      {
        inform(loc,"[MISRA C:2025 Rule 15.5] a function should have a single point of exit at the end\n");
        return_is_used = 0;
      }
      c_parser_consume_token (parser);
      if (c_parser_next_token_is (parser, CPP_SEMICOLON))
    ...
    case RID_TRANSACTION_ATOMIC:
      if(Wcontrolflow)
      inform(loc, "RID_TRANSACTION_ATOMIC");
    case RID_TRANSACTION_RELAXED:
      if(Wcontrolflow)
      inform(loc, "RID_TRANSACTION_RELAXED");
      stmt = c_parser_transaction (parser,
          c_parser_peek_token (parser)->keyword);
      break;
    case RID_TRANSACTION_CANCEL:
      if(Wcontrolflow)
      inform(loc, "RID_TRANSACTION_CANCEL");
      stmt = c_parser_transaction_cancel (parser);
      goto expect_semicolon;
    case RID_AT_THROW:
      if(Wcontrolflow)
      inform(loc, "RID_AT_THROW");
      gcc_assert (c_dialect_objc ());
      c_parser_consume_token (parser);
      if (c_parser_next_token_is (parser, CPP_SEMICOLON))
        {
    ...
    default:
      if(Wcontrolflow)
      inform(loc, "expr_stmt");
      goto expr_stmt;
    }
      break;
    case CPP_SEMICOLON:
      if(Wcontrolflow)
      inform(loc, "CPP_SEMICOLON");
      c_parser_consume_token (parser);
      break;
    case CPP_CLOSE_PAREN:
      if(Wcontrolflow)
      inform(loc, "CPP_CLOSE_PAREN");
    case CPP_CLOSE_SQUARE:
      if(Wcontrolflow)
      inform(loc, "CPP_CLOSE_SQUARE");
...
c_parser_condition (c_parser *parser)
{
  ...
  // check tree type for MISRA-C 14.4
  if (cond->typed.type->base.code == BOOLEAN_TYPE) {
    misra_14_4_bool_type = 1;
  } else if (cond->base.code == INTEGER_CST) {
	if (*(((struct tree_int_cst *)cond)->val) == 0 || *(((struct tree_int_cst *)cond)->val) == 1) {
		misra_14_4_bool_type = 1;
	}
  }
          ...
          const token_indent_info &if_tinfo)
{
  ...
  if (parser->tokens->type != CPP_OPEN_BRACE && Wmisra_c_trigger) {
        inform(parser->tokens->location,"[MISRA C:2025 Rule 15.6] the body of an iteration-statement or selection-statement shall be a compound statement\n");
  }
  ...
  if (c_parser_next_token_is (parser, CPP_SEMICOLON))
    {
      ...
      if (Wmisra_c_trigger) {
          inform(body_loc, "[MISRA C:2025 Rule 15.6] the body of an iteration-statement or selection-statement shall be a compound statement\n");   
      }
      location_t loc = c_parser_peek_token (parser)->location;
      warning_at (loc,
          OPT_Wempty_body,
...
c_parser_if_statement (c_parser *parser, bool *if_p, vec<tree> *chain)
{
  ...
  if (misra_14_4 != 10 && !misra_14_4_bool_type) {
        if (Wmisra_c_trigger) {
            inform(loc, "[MISRA C:2025 Rule 14.4] the controlling expression of an if or iteration statement shall be essentially Boolean\n");
        }
  }
  misra_14_4 = 0;
  misra_14_4_bool_type = 0;
  if(cond&&Wmisra_c_trigger)
  {
    int i=0;
    int len=TREE_OPERAND_LENGTH(cond);
    for(i=0;i<len;i++)
        if(TREE_CODE(TREE_OPERAND(cond,i))==MODIFY_EXPR&&loc)
            {
                inform(loc,"[MISRA C:2025 Rule 13.4] the result of an assignment operator should not be used\n");
             }  
  }
    ...
    *if_p = true;
    ...
    if(Wmisra_c_trigger ==1){
        if(second_body == NULL_TREE&&else_body == 1) {
                inform(loc,"[MISRA C:2025 Rule 15.7] all if...else if constructs shall be terminated with an else statement\n");
        }
    }
...
c_parser_switch_statement (c_parser *parser, bool *if_p)
{
  ...
  if (Wmisra_c_trigger && misra_index_16_5) {
	for (int i = 0; i < misra_index_16_5; i += 2) {
		if (misra_array_16_5[i] == 2 && !i || misra_array_16_5[i] == 2 && i == misra_index_16_5 - 2) {
			...
			inform(misra_array_16_5[i + 1], "[MISRA C:2025 Rule 16.5] a default label shall appear as either the first or the last switch label\n");
	...
	if (misra_index_16_5 < 4) {
		inform(misra_array_16_5[misra_index_16_5 - 1], "[MISRA C:2025 Rule 16.6] every switch statement shall have at least two switch-clauses\n");
		inform(switch_loc, "The switch is declaration in here\n");
	}
  ...
  if (!misra_default_16_4 && Wmisra_c_trigger) {
	inform(parser->tokens->location, "[MISRA C:2025 Rule 16.4] every switch statement shall have a default label\n");
	inform(switch_loc, "The switch is declaration in here\n");	
  }
  ...
  c_break_label = save_break;
    //printf("c_break_label:\n");
    //debug_tree(c_break_label);
...
c_parser_while_statement (c_parser *parser, bool ivdep, bool *if_p)
{
  ...
  if (misra_15_4 > 1 && Wmisra_c_trigger) {
        inform(loc, "[MISRA C:2025 Rule 15.4] there should be no more than one break or goto to terminate any iteration statement\n");
        //misra_15_4 = 0;
  }
  misra_15_4 = 0;
  c_break_label = save_break;
...
//////////////////////////////////////////////////////////////////////////13.4/////////////////////////////////////////////////
//check subtree of the tree node;return 1 if subtree has tree code: modify_expr

bool check_subtree_modify_expr (tree ret2)
{

    if(TREE_OPERAND_LENGTH(ret2) > 0)
    {
        for(int i = 0; i<TREE_OPERAND_LENGTH(ret2);i++)
        {
            if(TREE_CODE(TREE_OPERAND(ret2,i)) == MODIFY_EXPR)
            {
                return 1;
            }
        }
             ...
             tree omp_atomic_lhs)
{
  ...
  enum tree_code code;
    //debug_tree(omp_atomic_lhs);
        //debug_tree(after.original_type);
        //printf("NO_COMMAS 6511\n");//
  location_t op_location, exp_location, misra_12_1_location;
  gcc_assert (!after || c_dialect_objc ());
        //inform(lhs.get_start (),"lhs1"); //
  lhs = c_parser_conditional_expression (parser, after, omp_atomic_lhs);
  misra_12_1_location = 0;
  op_location = c_parser_peek_token (parser)->location;
        //inform(op_location,"op_location"); 
  switch (c_parser_peek_token (parser)->type)
    {
    case CPP_EQ:
      code = NOP_EXPR;
    ...
    default:
      return lhs;
        ...
        location_t loc13_4 = op_location;
        //printf("4\n");//
  c_parser_consume_token (parser);
        //printf("5\n");//
  exp_location = c_parser_peek_token (parser)->location;
        //inform(exp_location,"exp_location");
  if (parser->tokens->type != CPP_OPEN_PAREN) {
    misra_12_1_location = exp_location;
  }
  ...
  if (rhs.value->base.code == COND_EXPR) {
    if (misra_12_1_location && Wmisra_c_trigger) {
        inform(misra_12_1_location, "[MISRA C:2025 Rule 12.1] the precedence of operators within expressions should be made explicit\n");
    }
  }
  ...
  ret.original_type = NULL;
    if(check_subtree_modify_expr(ret.value)&&Wmisra_c_trigger)                          //13.4
      {
        inform(loc13_4,"[MISRA C:2025 Rule 13.4] the result of an assignment operator should not be used\n");
      }
    
    //printf("\nret:\n");
    //location_t start = ret.get_start ();
    if(Wcontrolflow)
    inform(op_location,"c_parser_expr_no_commas");
    //debug_tree(ret.value);
...
c_parser_conditional_expression (c_parser *parser, struct c_expr *after,
                 tree omp_atomic_lhs)
{
  c_token *misra_tmp_token;
  int misra_paren = 0;
  int misra_location;
  struct c_expr cond, exp1, exp2, ret;
  location_t start, cond_loc, colon_loc, middle_loc;
  gcc_assert (!after || c_dialect_objc ());
  misra_location = parser->tokens->location;
  ...
  if (misra_18_4 && Wmisra_c_trigger) {
      if (cond.value->typed.type->base.code == POINTER_TYPE) {
          inform(misra_location, "[MISRA C:2025 Rule 18.4] the +, -, += and -= operators should not be applied to an expression of pointer type\n");
      }
      //inform(parser->tokens->location, "[MISRA C:2025 Rule 18.4] the +, -, += and -= operators should not be applied to an expression of pointer type\n");
      misra_18_4 = 0;
  }
 ...
 /*
  if (parser->tokens->type == CPP_QUERY) {
    misra_tmp_token = parser->tokens;
    //c_parser_consume_token (misra_tmp_parser);
    //c_lex_one_token (misra_tmp_parser, &misra_tmp_parser->tokens[0]);
    misra_tmp_token--;
    while (misra_tmp_token->type != CPP_COLON) {
        if (misra_tmp_token->type == CPP_OPEN_PAREN) {
            misra_paren++;  
        }
        if (misra_tmp_token->type == CPP_CLOSE_PAREN) {
            misra_paren--;
        }
        if (misra_tmp_token->type == CPP_MINUS || misra_tmp_token->type == CPP_PLUS) {
            if (!misra_paren) {
                inform(cond_loc, "[MISRA C:2025 Rule 12.1] the precedence of operators within expressions should be made explicit\n");
            }
        }
        //c_lex_one_token (misra_tmp_parser, &misra_tmp_parser->tokens[0]);
        misra_tmp_token--;
    }   
    while (misra_tmp_token->type != CPP_SEMICOLON) {
        if (misra_tmp_token->type == CPP_OPEN_PAREN) {
                        misra_paren++;
                }
                if (misra_tmp_token->type == CPP_CLOSE_PAREN) {
  ...
  */
    ...
    }
  {
    ...
    if (exp2.value->base.code == MINUS_EXPR || exp2.value->base.code == PLUS_EXPR || exp2.value->base.code == MULT_EXPR ||
        exp2.value->base.code == TRUNC_DIV_EXPR || exp2.value->base.code == CEIL_DIV_EXPR || exp2.value->base.code == FLOOR_DIV_EXPR ||
    exp2.value->base.code == ROUND_DIV_EXPR || exp2.value->base.code == TRUNC_MOD_EXPR || exp2.value->base.code == CEIL_MOD_EXPR ||
    exp2.value->base.code == FLOOR_MOD_EXPR || exp2.value->base.code == ROUND_MOD_EXPR || exp2.value->base.code == RDIV_EXPR ||
    exp2.value->base.code == EXACT_DIV_EXPR || exp2.value->base.code == LT_EXPR || exp2.value->base.code == LE_EXPR ||
    ...
    if (!misra_paren && Wmisra_c_trigger) {
        inform(exp2_loc, "[MISRA C:2025 Rule 12.1] the precedence of operators within expressions should be made explicit\n");
        misra_paren--;
    }
    }
  ...
  if (Wmisra_c_trigger && cond.original_type != NULL) {
	if (cond.original_type->base.code != BOOLEAN_TYPE) {
		inform(start, "[MISRA C:2025 Rule 10.1] operands shall not be of an inappropriate essential type\n");
	}
  }
...
#define POP                                   \
  ...
  if (stack[0].expr.value->base.code == POSTINCREMENT_EXPR) {
	if (stack[0].expr.value->typed.type->base.code == BOOLEAN_TYPE) {
		inform(stack[0].loc, "[MISRA C:2025 Rule 10.1] operands shall not be of an inappropriate essential type\n");
	}
  }
  */
 ...
 out:
  while (sp > 0)
    POP;
    if(Wcontrolflow)    
    inform(binary_loc,"c_parser_binary_expression");
  return stack[0].expr;
#undef POP
}
...
c_parser_cast_expression (c_parser *parser, struct c_expr *after)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)
      && c_token_starts_typename (c_parser_peek_2nd_token (parser)))
    {
      ...
      /*
		(gdb) p type_name->specs->type->base
$66 = {code = BOOLEAN_TYPE, side_effects_flag = 0, constant_flag = 0, 
  addressable_flag = 0, volatile_flag = 0, readonly_flag = 0, 
  asm_written_flag = 0, nowarning_flag = 0, visited = 0, used_flag = 0, 
  ...
  protected_flag = 0, deprecated_flag = 0, default_def_flag = 0, u = {bits = {
      ...
      lang_flag_4 = 0, lang_flag_5 = 0, lang_flag_6 = 0, saturating_flag = 0, 
      unsigned_flag = 1, packed_flag = 0, user_align = 0, nameless_flag = 0, 
      atomic_flag = 0, spare0 = 0, spare1 = 0, address_space = 0}, 
    int_length = {unextended = 0 '\000', extended = 1 '\001', 
      offset = 0 '\000'}, length = 256, version = 256, ifn = 256, 
    dependence_info = {clique = 256, base = 0}}}
      */	
      c_parser_skip_until_found (parser, CPP_CLOSE_PAREN, "expected %<)%>");
      if (type_name == NULL)
    	{
	...
	if (expr.original_type) {
		if (type_name->specs->type->base.code != expr.original_type->base.code) 
			inform(cast_loc, "[MISRA C:2025 Rule 10.5] the value of an expression should not be cast to an inappropriate essential type\n");
	} else {
    		if (type_name->specs->type->base.code != expr.value->typed.type->base.code) {
			if (type_name->specs->type->base.code == BOOLEAN_TYPE && 
	    			expr.value->base.code == INTEGER_CST && 
    ...
    */
	...
	if (expr.original_type) {
		if (type_name->specs->type->type_common.precision > expr.original_type->type_common.precision && type_name->specs->type->base.code != VOID_TYPE)
                        inform(cast_loc, "[MISRA C:2025 Rule 10.8] the value of a composite expression shall not be cast to a different essential type category or a wider type\n");
...
c_parser_unary_expression (c_parser *parser)
{
  ...
  c_parser *misra_tmp_parser = parser;
  location_t op_loc = c_parser_peek_token (parser)->location;
  //location_t misra_loc = c_parser_peek_token(misra_tmp_parser)->location;
  location_t exp_loc;
  location_t finish;
  ret.original_code = ERROR_MARK;
  ret.original_type = NULL;
  ...
  // misra 12.1
  /* 
  if (misra_tmp_parser->tokens->type == CPP_KEYWORD && misra_tmp_parser->tokens->keyword == RID_SIZEOF) {
    while (c_parser_peek_token(misra_tmp_parser)->type != CPP_SEMICOLON) {
        if (c_parser_peek_token(misra_tmp_parser)->type == CPP_OPEN_PAREN) {
            misra_paren++;      
        }
        if (c_parser_peek_token(misra_tmp_parser)->type == CPP_CLOSE_PAREN) {
            misra_paren--;
        }
        if (c_parser_peek_token(misra_tmp_parser)->type == CPP_PLUS || c_parser_peek_token(misra_tmp_parser)->type == CPP_MINUS ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_MULT || c_parser_peek_token(misra_tmp_parser)->type == CPP_DIV ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_MOD || c_parser_peek_token(misra_tmp_parser)->type == CPP_AND ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_OR || c_parser_peek_token(misra_tmp_parser)->type == CPP_XOR ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_RSHIFT || c_parser_peek_token(misra_tmp_parser)->type == CPP_LSHIFT ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_AND_AND || c_parser_peek_token(misra_tmp_parser)->type == CPP_OR_OR ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_QUERY || c_parser_peek_token(misra_tmp_parser)->type == CPP_EQ_EQ ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_NOT_EQ || c_parser_peek_token(misra_tmp_parser)->type == CPP_GREATER_EQ ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_GREATER || c_parser_peek_token(misra_tmp_parser)->type == CPP_LESS ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_LESS_EQ || c_parser_peek_token(misra_tmp_parser)->type == CPP_PLUS_EQ ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_MINUS_EQ || c_parser_peek_token(misra_tmp_parser)->type == CPP_MULT_EQ ||
            c_parser_peek_token(misra_tmp_parser)->type == CPP_DIV_EQ || c_parser_peek_token(misra_tmp_parser)->type == CPP_MOD_EQ ||
            ...
            inform(op_loc, "[MISRA C:2025 Rule 12.1] the precedence of operators within expressions should be made explicit\n");
            break;
        }
        c_parser_consume_token(misra_tmp_parser);
    }
...
c_parser_sizeof_expression (c_parser *parser)
{
  ...
  in_sizeof++;
  /*
  if (c_parser_next_token_is(misra_tmp_parser, )) {
  ...
  */
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN)
    ...
    //printf("enter c-parser.c line:7157\n");
      struct c_type_name *type_name;
      c_parser_consume_token (parser);
  ...
  if (et_ctrl != et_std)
    {
      warning_at (generic_loc, 0,
                  "[MISRA C:2025 Rule 23.6] the type of the controlling expression and the generic association type shall have the same essential type category");
    }
}
                             ...
                             auto_vec<c_generic_association> *associations)
{
  ...
  */
  ...
  if (used_count > 0 && unused_count > 0)
    {
      warning_at (generic_loc, 0,
                  "[MISRA C:2025 Rule 23.7] all generic associations in a _Generic selection shall be used");
    }
}
  ...
  if (!in_system_header_at (generic_loc))
    {
      ...
      if (!from_macro_expansion_at (generic_loc))
        warning_at (generic_loc, 0,
                    "[MISRA C:2025 Rule 23.1] operands of the _Generic keyword shall not be compatible types");
    }
        ...
        if (control_t && misra_expr_has_potential_side_effects (control_t))
          {
            warning_at (control_loc, 0,
              "[MISRA C:2025 Rule 23.2] the result of a _Generic expression shall be used");
          }
      }
  }
/* ===== 以上為新增 ===== */
      ...
      if (assoc.type
          ...
          && misra234_is_forbidden_assoc_type (assoc.type))
        {
          ...
          warning_at (assoc.type_location, 0,
                      "[MISRA C:2025 Rule 23.4] a _Generic selection shall not have a function or array type as a generic association");
        }
...
//fprintf(stderr,"5\n");
  ...
  if (!in_system_header_at (generic_loc)          /* 不檢查系統標頭 */
    ...
    {
      warning_at (generic_loc, 0,
                  "[MISRA C:2025 Rule 23.8] a default generic association shall appear last in a _Generic selection");
  ...
  if (!in_system_header_at (generic_loc)
    ...
    {
      warning_at (generic_loc, 0,
                  "[MISRA C:2025 Rule 23.3] no two _Generic associations shall specify a compatible type");
    }
      ...
      for (ix = 0; associations.iterate (ix, &iter); ++ix)
        {
          ...
          if (misra_pointer_implicitly_convertible_p (selector_type,
            ...
            {
              warning_at (generic_loc, 0,
                          "[MISRA C:2025 Rule 23.5] the first operand of _Generic shall not be compatible with any generic association type");
              /* 一個 _Generic 只報一次就好 */
              break;
            }
        }
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    if (!preceded_by_amp
      ...
      warning_at (loc,
                  ...
                  "[MISRA C:2025 Rule 17.12] the va_arg macro shall only be invoked for an argument of an appropriate type");
...
static struct c_expr
c_parser_expression (c_parser *parser)                              //C_PARS_EXPR ->C_PARS_NO_COMMAS
{
//printf("C_PARS_EXPR 8775\n");
...
//fprintf(stderr,"7\n");
  if (c_parser_next_token_is (parser, CPP_COMMA)) {
	...
	inform(parser->tokens->location, "[MISRA C:2025 Rule 12.3] the comma operator should not be used\n");
  }
  while (c_parser_next_token_is (parser, CPP_COMMA))
    {
      struct c_expr next;
...
{
//printf("c_parser_expression_conv 8811\n");
  struct c_expr expr;
  location_t loc = c_parser_peek_token (parser)->location;
  expr = c_parser_expression (parser);
    //debug_tree(expr.value);
             ...
             tree array_value)
{
  ...
  if (value_tree != error_mark_node)
    ...
    //fprintf(stderr, "value_tree\n")
    //debug_tree(value_tree);
  return value_tree;
}
