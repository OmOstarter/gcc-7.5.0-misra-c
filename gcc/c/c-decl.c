...
void hash_initialize(void)
{
	for(int i=0;i<HASH_SIZE;i++)
	 	if(HASH_TABLE[i].used_flag>=3)
			HASH_TABLE[i].used_flag = 0;
}
void hash_search_func(char *key,int value,int level,location_t locus)
{
	while(HASH_TABLE[value].used_flag!=0)
	{
		if(strcmp(HASH_TABLE[value].key,key)==0)
		{
			if(HASH_TABLE[value].used_flag==1)
				inform(locus,"\n==========================Misra-c 2012 rule violation:5.5==========================\n"
				"Rule 5.5: Identifiers shall be distinct from macro names\n"
				"Category: Required\n");
			else if(HASH_TABLE[value].used_flag==level+1)
				inform(locus,"\n==========================Misra-c 2012 rule violation:5.2==========================\n"
				"Rule 5.2: Identifier s declared in the same scope and name space shall be distinct\n"
				"Category: Required\n");
			else
				inform(locus,"\n==========================Misra-c 2012 rule violation:5.3==========================\n"
	...
	strcpy(HASH_TABLE[value].key,key);
	HASH_TABLE[value].used_flag=level+1;
	return ;
}
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 ...
	 update.  */
      ...
      if (Wmisra_c_trigger) {
		...
		if (p->scope->bindings->locus) {
			inform(p->scope->bindings->locus, "Misra 15.3\n");	
		}	
      	}	      
      }
      */
...
{
  struct c_scope *s;
  int misra_tmp_label_scope = 0;
  int misra_tmp_scope_depth = 0;
  struct c_binding *misra_label_bind;
  unsigned int misra_tmp_label_decl_locus = 0;
  s = scope;
  while (s != NULL)
    {
	  ...
	  for (b = s->bindings; b != NULL; b = b->prev)
	    {
		 ...
		 if (current_scope->depth > s->depth) {
		...
		if (label_vars->label_bindings.scope != NULL)
			...
			inform(DECL_SOURCE_LOCATION(b->decl), "The label is define in here scope is %d against the Misra-C 15.3\n", b->depth);
		        //locate_old_decl(b->decl);	
		}
	      }	      
  ...
  for (b = scope->bindings; b; b = free_binding_and_advance (b))
	...
	case LABEL_DECL:
	  /* Warnings for unused labels, errors for undefined labels.  */
		//if(DECL_INITIAL (p)){debug_tree(p);}
	  ...
	  if (Wmisra_c_trigger) {
	  	if (p != block) {
			//fprintf(stderr, "Misra-C: 15.3\n");
			inform(b->locus, "Misra-C: 15.3\n");
	  	}
	  }
	  */
	...
	case VAR_DECL:
	  ...
	  if ((!TREE_USED (p) || !DECL_READ_P (p))
	      ...
	      && scope != external_scope)
	    {
	      if (!TREE_USED (p)) {
		if (Wmisra_c_trigger) {
			inform(b->locus, "Misra-C Rule 2.2\n");
		}
  ...
  /* Function declarations can either be 'static' or 'extern' (no
	  ...
	  && TREE_PUBLIC (newdecl) != TREE_PUBLIC (olddecl))
	{
	  if (DECL_EXTERNAL (newdecl))
		...
		error ("Misra-c Rule 8.8 static declaration of %q+D follows "
		       "non-static declaration", newdecl);
...
static const char *
header_for_builtin_fn (enum built_in_function fcode,location_t loc)
{
  switch (fcode)
    {
      ...
      else
	{
	  ...
	  if (DECL_BUILT_IN (decl))
	      ...
	      if (!comptypes (newtype, TREE_TYPE (decl)))
		{
		  ...
		  const char *header
		    = header_for_builtin_fn (DECL_FUNCTION_CODE (decl),loc);
                  //loc_misra_c = loc;
...
bool NUL_label = 0; //check if the tree label has been initialized,1 means not initialized
	    ...
	    bool initialized, tree attributes)
{
  ...
  if (initialized)
	...
	if (TREE_TYPE (decl) == error_mark_node)
	...
	else if (COMPLETE_TYPE_P (TREE_TYPE (decl)))
	  {
		...
		inform(DECL_SOURCE_LOCATION(decl),"\n==========================Misra-c 2012 rule violation:18.8==========================\n"
	...
	else if (C_DECL_VARIABLE_SIZE (decl))
	  {
	    ...
	    error ("variable-sized object may not be initialized3");
	    initialized = 0;
	  }
      }
  ...
  if ((TREE_CODE (decl) == FUNCTION_DECL || TREE_CODE(decl)==TYPE_DECL)
      && targetm.calls.promote_prototypes (TREE_TYPE (decl)))
    {
      struct c_declarator *ce = declarator;

      if (ce->kind == cdk_pointer && TREE_CODE(decl)!=TYPE_DECL)
      ...
      if (ce->kind == cdk_function)
	{
	  ...
	  if(!ce->u.arg_info->types)
		...
		if(Wmisra_c_trigger)
		inform(DECL_SOURCE_LOCATION(decl),"\n==========================Misra-c 2012 rule violation:8.2==========================\n"
	  ...
	  for (; args; args = DECL_CHAIN (args))
	    {
		  ...
		  && TYPE_PRECISION (type) < TYPE_PRECISION (integer_type_node) && TREE_CODE(decl)!=TYPE_DECL)
...
{
  tree type;
  tree misra_18_5_pointer_check;
  int misra_pointer_counter = 0;
  bool was_incomplete = (DECL_SIZE (decl) == 0);
  const char *asmspec = 0;
  if (decl->base.code == VAR_DECL && Wmisra_c_trigger && decl->typed.type->base.code == POINTER_TYPE) {
	misra_18_5_pointer_check = decl->typed.type;
	while (misra_18_5_pointer_check->base.code == POINTER_TYPE) {
		if (misra_18_5_pointer_check->base.code == POINTER_TYPE) {
			misra_pointer_counter++;
		}
		misra_18_5_pointer_check = misra_18_5_pointer_check->typed.type;
	}
	if (misra_pointer_counter > 2) {
		inform(((struct tree_decl_minimal *)decl)->locus, "Misra-c Rule 18.5\n");
	}
			       ...
			       tree orig_name)
{
  ...
  if (type_mv != unsigned_type_node) {
	inform(loc, "Misra-c Rule 6.2 bit fields need unsigned type\n");	
  }
  if (type_mv == long_integer_type_node) {
	inform(loc, "Misra-c Rule 6.1 even if long and int are the same size\n");	
  }
  if (type_mv == integer_type_node) {
	inform(loc, "Misra-c Rule 6.1 plain int not permitted\n");
  }
...
static tree
grokdeclarator (const struct c_declarator *declarator,								//17.6
		...
		enum deprecated_states deprecated_state)
{
  ...
  int misra_pointer_level_checker = 0;
  ...
  *expr_const_operands = declspecs->expr_const_operands;
     ...
     and get it as an IDENTIFIER_NODE, for an error message.  */
  {
    ...
    const struct c_declarator *misra_decl = decl;
	...
	case cdk_array:
	  ...
	  /* FALL THRU.  */
  	  /*
	  if (Wmisra_c_trigger && misra_decl->declarator) {
		
	  }
	  */
	case cdk_function:
	case cdk_pointer:
	  if (Wmisra_c_trigger && misra_decl->kind == cdk_pointer) {
		misra_pointer_level_checker++;
		misra_decl = misra_decl->declarator;
		while (misra_decl) {
			if (misra_decl->kind == cdk_pointer) {
				misra_pointer_level_checker++;
			} else {
				break;
			}
		...
		if (misra_pointer_level_checker > 2 && !misra_decl->id_loc) {
		  ...
		  struct c_enum_contents *the_enum, tree name, tree value)
{
  ...
  //misra_enum_array_08_12[misra_enum_index++] = (long long int)the_enum;
