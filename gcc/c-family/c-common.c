...
fix_string_type (tree value)
{
  ...
  if (c_dialect_cxx() || warn_write_strings || OPT_Wmisra_c)
...
unsafe_conversion_p (location_t loc, tree type, tree expr, bool produce_warns)
{
  ...
  //tree misra_type = TREE_TYPE (type);
		       ...
		       int only_writes)
{
  ...
  while (list)
    {
		//printf("candidate_equal_p (list->expr, written) = %d\n\n",candidate_equal_p (list->expr, written));
		//printf("candidate_equal_p (list->writer, writer) = %d\n\n",candidate_equal_p (list->writer, writer));
		//printf("only_writes = %d\n\n",only_writes);
		//printf("list->writer = %d\n\n",list->writer);
      if (candidate_equal_p (list->expr, written)
	  && !candidate_equal_p (list->writer, writer)
	  && (!only_writes || list->writer))
	{printf("enter c-common.c 1665\n");
...
check_user_alignment (const_tree align, bool allow_zero)
{
  ...
  if (TREE_CODE (align) == INTEGER_CST) {
    ...
    if (value == 0) {
        warning_at (input_location, OPT_Wmisra_c, "MISRA C:2025 Rule 8.16");
    }
}
				 ...
				 int required)
{
  if (nargs < required)
    {
      error_at (loc, "too few arguments to function BFVN %qE", fndecl);
      return false;
    }
...
sync_resolve_size (tree function, vec<tree, va_gc> *params, bool fetch)
{
  ...
  if (vec_safe_is_empty (params))
    {
      error ("too few arguments to function SRS %qE", function);
      return 0;
    }
		     ...
		     vec<tree, va_gc> *params, bool orig_format)
{
  ...
  while (1)
	...
	{
	  error_at (loc, "too few arguments to function SRP %qE", orig_function);
	  return false;
	}
